...
Class UpFile_Cls
	...
	Private Function FormatName(Byval FileExt)
		...
		TempStr = Day(Now) & Hour(Now) & Minute(Now) & RanNum & "." & FileExt
		If RName_Str<>"" Then
			TempStr = RName_Str & TempStr
		End If
		FormatName = TempStr
		If FileNameByID <> "" Then
			FormatName = FileNameByID & "_" & FormatName
		End if
	End Function
	...
	Private Function CheckFileExt(FileExt)
		...
		If FileExt="" or IsEmpty(FileExt) Then
			...
			Exit Function
		End If
		If FileExt="asp" or FileExt="asa" or FileExt="aspx" or FileExt="shtml" or FileExt="shtm" Then
			CheckFileExt = False
			Exit Function
		End If
		'AT20060615
		'Response.Write FileExt & "-" & InceptFile
		'Response.End
	...
	'-----------------------------------------------------------------------------------
	'判断文件类型:0=其它,1=图片,2=FLASH,3=音频,4=视频,5=压缩文件,6=文档文件
	'-----------------------------------------------------------------------------------
	Private Function CheckFiletype(Byval FileExt)
		FileExt = Lcase(Replace(FileExt,".",""))
		select Case FileExt
				Case "gif", "jpg", "jpeg","png","bmp","tif","iff","psd","pcx"
					CheckFiletype = 1
				Case "swf", "swi","flv"
					CheckFiletype = 2
				Case "mid", "wav", "mp3","rmi","cda","ra","ram","wma"
					CheckFiletype = 3
				Case "avi", "mpg", "mpeg","wov","asf","rmvb","rm","mp4","3gp","mov","wmv"
					CheckFiletype = 4
				Case "rar","zip","cab","iso","jar","7z","arj","sit"
					CheckFiletype = 5
				Case "doc","docx","docm","wps"
					CheckFiletype = 6
				Case Else
					CheckFiletype=0
		End select
