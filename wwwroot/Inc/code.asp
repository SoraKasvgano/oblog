...
Const nPositionXRandom = 30			' 横向位置随机量
Const nPositionYRandom = 20			' 纵向位置随机量
Const nAngleRandom = 6         	' 笔画角度随机量
Const nLengthRandom = 6        	' 笔画长度随机量(百分比)
Const nColorHue = -1						' 显示验证码的色调(-1表示随机色调, -2表示灰度色调)
Const cCharSet = "123I4E6HJ9KL5R7TXY9Z"          ' 构成验证码的字符集
...
OBASN=request("s")
	If OBASN<>"" And  session("Ob_Ask_Shake_hands"&OBASN) <>"" And InStr(session("Ob_Ask_Shake_hands"&OBASN),"|")  Then 
		If IsNumeric(Split(session("Ob_Ask_Shake_hands"&OBASN),"|")(1)) Then 
			If Split(session("Ob_Ask_Shake_hands"&OBASN),"|")(0)=OBASN And Int(Split(session("Ob_Ask_Shake_hands"&OBASN),"|")(1)) < Int(aTestMax) Then 
				Call SubRbdcode()

			End If 
		End If 
	ElseIf request("n")>0 Then 
		Call SubRbdcode()
	Else 
...
Sub SubRbdcode()
				Response.clear 
				Rndcode(OBASN)
				Response.Write "document.getElementById('ob_codeimg').innerHTML='"&session("OblogGetCode2_ask_"&OBASN)&"<br/>(请将答案填入验证码输入框.)'"
				
				Session("Ob_Ask_Shake_hands"&OBASN)= Split(session("Ob_Ask_Shake_hands"&OBASN),"|")(0) & "|" & (Int(Split(session("Ob_Ask_Shake_hands"&OBASN),"|")(1))+1)
				Response.End 
End Sub 
Sub CDGen_Reset()
	' 复位矢量笔和环境变量
...
Sub CDGen_Char(cChar, X0, Y0)
	...
	' 以下为字母的矢量动作，有兴趣的可以继续
	Case "A"
		CDGen_SetDirection -(Rnd * 20 + 152)              ' 逆时针150-170度(相对于垂直线)
		CDGen_FowardDraw Rnd * 0.2 + 1.2                  ' 绘制1.2-1.4个单位
		CDGen_SetDirection Rnd * 20 + 135                 ' 顺时针140-160度
		CDGen_FowardDraw Rnd * 0.2 + 1.4                  ' 绘制1.2-1.4个单位
