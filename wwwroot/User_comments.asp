...
Sub main()
	...
	tsql = " comment_user = '"&oblog.l_uname&"' AND isguest = 0 AND isdel = 0"
    sGuide = ""
    select Case cmd
        Case 0
            sql="select "&ssql&" from [oblog_comment] where userid="&oblog.l_uid&" AND isdel = 0 order by commentid desc"
            sGuide = sGuide & "最新500篇评论"
        Case 1
            sql="select "&ssql&" from [oblog_comment] where userid="&oblog.l_uid&" AND isdel = 0 order by commentid desc"
            sGuide = sGuide & "我文章里的评论"
        Case 2
            sql="select "&ssql&" from [oblog_comment] where userid="&oblog.l_uid&" and iState=1 AND isdel = 0 order by commentid desc"
            sGuide = sGuide & "已审核的评论"
        Case 3
            sql="select "&ssql&" from [oblog_comment] where userid="&oblog.l_uid&" and iState=0  AND isdel = 0 order by commentid desc"
            sGuide = sGuide & "待审核的评论"
        Case 4
            sql="select "&ssql&" from [oblog_comment] where "&tsql&" order by commentid desc"
...
<table id="TableBody" cellpadding="0">
	<thead>
		...
		<tr class="thead_tr2">
			<th>
				<table id="CommentsTop" class="ListTop" cellpadding="0">
					<tr>
						<td class="t1"></td>
		...
		<tr>
			<td>
				<div id="chk_idAll">
					...
					<table id="Comments" class="TableList" cellpadding="0">
						<%
						'Do while not rs.EOF
						...
						<tr id="u<%=rs("commentid")%>"  onclick="chk_iddiv('<%=rs("commentid")%>')">
							...
							<td class="t2">
								<a href="go.asp?logid=<%=rs("mainid")%>#<%=rs("commentid")%>" target="_blank" title="cssbody=[dvbdy1] cssheader=[dvhdr1] body=[<%=oblog.filt_html(FilterUbb(rs("comment")))%>]"><%If rs("iState") = 0 Then %>[待审]<%End if%><%=oblog.filt_html(rs("commenttopic"))%></a><br />
							...
							<td class="t4">
								<%If rs("userid") = oblog.l_uid Then %>
									<a href="user_comments.asp?action=modify&id=<%=rs("commentid")%>&re=true"><span class="blue">回复</span></a>&nbsp;
									<a href="user_comments.asp?action=modify&id=<%=rs("commentid")%>"><span class="green">修改</span></a>&nbsp;
									<a href="user_comments.asp?action=del&id=<%=rs("commentid")%>" onClick="return confirm('确定要删除此评论吗？');"><span class="red">删除</span></a>
								<%Else%>
...
%>
	<tfoot>
		<tr>
			<td>
				<form class="Search" id="CommentsSearch" action="user_comments.asp" name="form1" method="get">
...
<table id="TableBody" cellpadding="0">
	<thead>
		<tr class="thead_tr1">
			<th>
				<ul id="UserMenu">
					<li><a href="#" onclick="purl('user_comments.asp','评论管理')">评论管理</a></li>
				</ul>
			</th>
		</tr>
	</thead>
...
Sub Savemodify()
   ...
   if Request("re")="true" then
    ...
    Set blog = Nothing
    oblog.ShowMsg "修改评论成功！", "user_comments.asp"
End Sub
...
Sub delonecomment(id)
    ...
    sql = "select userid,mainid,istate from [oblog_comment] where commentid=" & CLng(id) & " and userid=" & oblog.l_uId
    Set rs = Server.CreateObject("adodb.recordset")
    rs.Open sql, conn, 1, 3
    If Not rs.EOF Then
        uid = rs(0)
        ...
        '重新计算评论数目
        Set rstComment = Server.CreateObject("adodb.recordset")
        rstComment.Open "select Count(commentid) From [oblog_comment] Where isdel=0 AND istate=1 AND mainid=" & CLng(mainid), conn, 1, 1
        ...
        rstComment.Open "select commentnum,scores From [oblog_log] where logid=" & mainid,conn,1,3
        rstComment(0)=CommentNum
        If istate=1 Then
	        If rstComment(1)>Int(oblog.CacheScores(6)) Then
	        	rstComment(1)=rstComment(1)-oblog.CacheScores(6)
	        Else
	        	rstComment(1)=0
	        End If
	    End If
