...
<div id="main_body">
	<ul class="main_top">
		<li class="main_top_left left"><%=tName%>分类管理</li>
  ...
  <table width="98%" border="0" align="center" cellpadding="2" cellspacing="1" Class="border">
    ...
    <td height="30"><a href="admin_logclass.asp?t=<%=t%>"><%=tName%>分类管理首页</a> |
...
%>
  <tr class="tdbg" onMouseOut="this.style.backgroundColor=''" onMouseOver="this.style.backgroundColor='#cccccc'" style="padding: 0px 2px;">
	...
	if iDepth>0 then
	  ...
	  Response.write "<a href='admin_logclass.asp?Action=Modify&id=" & rsClass("id") & "&t=" & t & "' title='" & rsClass("ReadMe") & "'>" & rsClass("classname") & "</a>"
	  if rsClass("Child")>0 then
    ...
    <td align="center">
    	...
    	<a href="admin_logclass.asp?Action=Modify&id=<%=rsClass("id")%>&t=<%=t%>">修改设置</a> |
      	<a href="admin_logclass.asp?Action=Move&id=<%=rsClass("id")%>&t=<%=t%>">移动分类</a> |
      	<a href="admin_logclass.asp?Action=Del&id=<%=rsClass("id")%>&t=<%=t%>" onClick="<%if rsClass("Child")>0 then%>return ConfirmDel1();<%else%>return ConfirmDel2();<%end if%>">删除</a></td>
  </tr>
  <%
	rsClass.movenext
loop
...
<form name="form1" method="post" action="admin_logclass.asp?&t=<%=t%>">
    ...
    <tr class="tdbg">
      <td height="40" colspan="2" align="center"><input name="t" type="hidden" id="t" value="<%=t%>"><input name="Action" type="hidden" id="Action" value="SaveMove">
...
sub UpOrder()
	...
	sqlOrder="select * From oblog_logclass where ParentID=0 and RootID<" & cRootID & " AND idType = "&t&" order by RootID desc"
