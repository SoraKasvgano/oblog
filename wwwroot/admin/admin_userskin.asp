...
sClasses= sClasses & "<option value=0>------尚未分类------</option>" & vbcrlf
...
sub showskin()
	...
	G_P_FileName="admin_userskin.asp?action=showskin"
	if ispass=1 then
...
sub saveconfig()
	...
	set rs=nothing
	ReCountSkins
	EventLog "进行了修改用户模板操作（文本方式），目标模板ID："&Request.QueryString("id")&"",oblog.NowUrl&"?"&Request.QueryString
	oblog.ShowMsg "保存成功",""
end sub
...
sub delconfig()
	...
	ReCountSkins
	EventLog "进行了删除用户模板操作，目标模板ID："&id&"",oblog.NowUrl&"?"&Request.QueryString
	oblog.ShowMsg "删除成功",""
end sub
sub modiconfig()
	dim rs
...
sub saveaddskin()
	...
	set rs=nothing
	ReCountSkins
	EventLog "进行了添加用户模板操作",oblog.NowUrl&"?"&Request.QueryString
...
Sub ReCountSkins()
	...
	'重新计数
	rst.Open "select classid From oblog_skinclass WHERE itype = 0",conn,1,3
	...
	Do While Not rst.Eof
		...
		If Not rst1.Eof Then
			oblog.Execute "Update oblog_skinclass Set icount=" & rst1(0) & " WHERE itype = 0 AND classid=" & rst(0)
		Else
			oblog.Execute "Update oblog_skinclass Set icount=0 WHERE itype = 0 AND classid=" & rst(0)
		End If
		rst.MoveNext
	Loop
	oblog.execute "Update oblog_userskin Set classid=0 Where classid Not In (select classid from oblog_skinclass WHERE itype = 0 )"
	Set rst=Nothing
	Set rst1=Nothing
End Sub
...
Sub SaveClass()
	...
	If classname<>"" Then
		Set rst=Server.CreateObject("ADODB.Recordset")
		rst.Open "select * From oblog_skinclass Where itype=0 AND classname='" & classname & "'",conn,1,3
		'是否重名
		If Not rst.Eof Then
			rst.Close
			Set rst=Nothing
			'oblog.ShowMsg "目标名称与现有分类名称重复","admin_userskin.asp?action=skinclass"
		...
		Else
			rst.Close
			rst.Open "select * From oblog_skinclass Where itype=0 AND classid=" & classid ,conn,1,3
		End If
...
Sub DelClass()
	...
	If rst("icount")>0 Then
		rst.Close
		Set rst=Nothing
		oblog.ShowMsg "目标分类中有模板数据,请将该分类中的模板转移到其他分类然后再删除","admin_userskin.asp?action=skinclass"
	Else
		rst.Delete
		Set rst=Nothing
		oblog.execute("update oblog_userskin Set classid=0 where classid=" & classid)
		oblog.ShowMsg "分类操删除成功","admin_userskin.asp?action=skinclass"
	End If
End Sub
Set oblog = Nothing
%>