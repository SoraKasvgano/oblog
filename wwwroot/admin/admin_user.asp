...
<%
dim rs, sql,rsGroup,sGroups,allGroups,sClass,rsClass
...
<!--           <option value="7">等待管理员认证的用户</option> -->
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;<a href="admin_user.asp">用户管理首页</a>&nbsp;|&nbsp;<a href="admin_user.asp?cmd=6">待审核用户</a>|&nbsp;<a href="admin_user.asp?cmd=9"><font color=red>敏感用户</font></a>|&nbsp;<a href="admin_user.asp?cmd=10" >被前台屏蔽的用户</a>|&nbsp;<a href="../reg.asp" target="_blank">添加新用户</a></td>
    </tr>
  </form>
...
Sub main()
    ...
    select Case cmd
        ...
        Case 101
            ...
            Else
                sql = "select   " & QryFields &"  from oblog_user where  user_group=" & clng(Request("groupid"))
            End If
            ...
            Else
                ...
                Case "regdate"
                	...
                	If Keyword<>"" Then
                        sql = "select  " & QryFields &"  from oblog_user where "
						If Is_Sqldata = 0 Then
							sql = sql & " datediff(" & G_Sql_d & ",adddate,'" & keyword & "')=0 Order By userid Desc "
						Else
							sql = sql & " adddate = '"&sDate&"'  ORDER BY userid DESC "
						End if
                ...
                Case "birthday"
                	...
                	If Keyword<>"" Then
                        sql = "select  " & QryFields &"  from oblog_user where "
						If Is_Sqldata = 0 Then
							sql = sql & " datediff(" & G_Sql_d & ",birthday,'" & keyword & "')=0  Order By userid Desc "
						Else
							sql = sql & " birthday = '"&sDate&"' ORDER BY userid DESC "
						End if
                ...
                Case "regcity"
                	...
                	If InStr(keyword,",")>0 Then
                		aCity=Split(keyword,",")
                		sql = "select  " & QryFields &"  from oblog_user where province='" & aCity(0)&"' And city='"& aCity(1)&"' order by userID  desc"
	...
	If IsEmpty(sql) Then
		Response.write "<script language=JavaScript>alert(""请正确填写查询参数"");history.go(-1)</script>"
		Exit Sub
	End if
'	OB_DEBUG sql,1
...
<!--     <td align="center">
    ...
    %>&nbsp;
      ...
      If rs("user_level") = 6 Then
      	Response.Write "<span><a href='admin_user.asp?Action=pass&UserID=" & rs("userID") & "' title=""通过审核"">审核</a></span>  "
      Else
      	Response.Write "<span><a href='admin_user.asp?Action=unpass&UserID=" & rs("userID") & "' title=""取消审核"">取消</a></span>  "
      End If
        %>
	<br />
	  <%
    ...
    <td align="center">
        ...
        If CheckAccess("r_user_admin") Then
	        Response.Write "<a href='admin_user.asp?Action=gouser2&username=" & rs("username") & "' target='_blank'>进后台</a>&nbsp;"
	    End If
        Response.Write "<a href='admin_user.asp?Action=Del&UserID=" & rs("userID") & "' onClick='return confirm(""确定要删除此用户吗？"");'>删除</a><br/>"
		If CheckAccess("r_user_admin") Then
			If rs("is_log_default_hidden") = 1 Then
				Response.Write "<a href=""admin_user.asp?Action=Unblock&UserID=" & rs("userID") & """ style=""color:red;"">取消屏蔽</a>&nbsp;"
			Else
				Response.Write "<a href=""admin_user.asp?Action=block&UserID=" & rs("userID") & """ style=""color:green;"">前台屏蔽</a>&nbsp;"
			End If
		End If
        %>
	</td>
...
<FORM name="Form1" action="admin_user.asp" method="post">
  <table width="98%" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#FFFFFF" class="border">
    <TR class="tdbg" >
      <TD width="40%">用户名：</TD>
      <TD width="60%"><%=rsUser("userName")%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	  <input name="userName" type="hidden" id="userName" value="<%=rsUser("userName")%>">
	  </TD>
    </TR>
...
Sub SaveModify()
    ...
    Dim userid, userName, Password, PwdConfirm, Question, Answer, Sex, Email, Homepage, OICQ, MSN, User_Level, LockUser, isbest
	...
	If oblog.cacheConfig(51)="1"  Then
		...
		If  sMobile<>"" Then
			If Len(sMobile) = 11 And IsNumeric(sMobile) Then
	        	If CInt(Left(sMobile, 3)) >= 135 And CInt(Left(sMobile, 3)) <= 139 Then
	            	'bMobile = True
	            Else
	...
	Else
		oblog.Execute ("UPDATE oblog_log SET is_log_default_hidden = 0 WHERE userid = "&userid&" or authorid = "&userid)
	End If
	Session ("CheckUserLogined_"&userName)=""
	oblog.CountGroupUser
	EventLog "进行了修改用户资料操作，目标用户ID："&userid&"",oblog.NowUrl&"?"&Request.Form
    oblog.ShowMsg "修改成功!", ""
End Sub
...
Sub DelUser()
	...
	EventLog "进行了删除用户操作，目标用户ID："&userid&"",oblog.NowUrl&"?"&Request.QueryString
'	Response.Redirect "admin_user.asp"
    oblog.ShowMsg "用户删除成功!",""
End Sub
...
Sub LockUser()
    ...
    Set rs = Nothing
	oblog.Execute ("UPDATE oblog_log SET is_log_default_hidden = 1 WHERE userid = "&userid&" or authorid = "&userid)
	EventLog "进行了锁定用户操作，目标用户ID："&userid&"",oblog.NowUrl&"?"&Request.QueryString
...
Sub UnLockUser()
    ...
    userid = CLng(userid)
    Set rs = Server.CreateObject("adodb.recordset")
    rs.open "select lockuser,istrouble,is_log_default_hidden from oblog_user where userid=" & userid, Conn, 1, 3
    If Not rs.EOF Then
        rs(0) = 0
		rs(1) = 0
		rs(2) = 0
    ...
    Set rs = Nothing
	oblog.Execute ("UPDATE oblog_log SET is_log_default_hidden = 0 WHERE userid = "&userid&" or authorid = "&userid)
	EventLog "进行了解锁用户操作，目标用户ID："&userid&"",oblog.NowUrl&"?"&Request.QueryString
...
Sub BlockUser()
    ...
    userid = CLng(userid)
    Set rs = Server.CreateObject("adodb.recordset")
    rs.open "select is_log_default_hidden from oblog_user where userid=" & userid, Conn, 1, 3
    If Not rs.EOF Then
        rs(0) = 1
        rs.Update
    End If
    rs.Close
    Set rs = Nothing
	oblog.execute("update oblog_log set is_log_default_hidden = 1 where userid=" & userid)
	EventLog "进行了屏蔽用户系统首页显示操作，目标用户ID："&userid&"",oblog.NowUrl&"?"&Request.QueryString
...
Sub UnBlockUser()
    ...
    rs.Close
    Set rs = Nothing
	oblog.execute("update oblog_log set is_log_default_hidden = 0 where userid=" & userid)
	EventLog "进行了解锁用户系统首页显示操作，目标用户ID："&userid&"",oblog.NowUrl&"?"&Request.QueryString
    oblog.ShowMsg "解锁用户成功", ""
End Sub
'审核
Sub PassIt(t)
    ...
    userid = CLng(userid)
    Set rs = Server.CreateObject("adodb.recordset")
    rs.open "select user_level from oblog_user where userid=" & userid, Conn, 1, 3
    If Not rs.EOF Then
        rs(0) = t
        rs.Update
    End If
    ...
    If t=6 Then
		oblog.Execute ("UPDATE oblog_log SET is_log_default_hidden = 1 WHERE userid = "&userid&" or authorid = "&userid)
		EventLog "进行了取消用户审核操作，目标用户ID："&userid&"",oblog.NowUrl&"?"&Request.QueryString
    	oblog.ShowMsg "取消用户审核成功", ""
    Else
		oblog.Execute ("UPDATE oblog_log SET is_log_default_hidden = 0 WHERE userid = "&userid&" or authorid = "&userid)
		EventLog "进行了用户审核操作，目标用户ID："&userid&"",oblog.NowUrl&"?"&Request.QueryString
...
Sub MoveUser()
	...
	oblog.CountGroupUser
'	Response.Redirect "admin_user.asp"
	EventLog "进行了用户组转移操作，目标用户组ID："&User_Group&"",oblog.NowUrl&"?"&Request.Form
    oblog.ShowMsg "用户转移成功!",""
...
Sub DoUpdate()
	...
	EventLog "进行了更新用户静态页面操作，开始用户ID："&BeginID&"，结束用户ID："&EndID&"",oblog.NowUrl&"?"&Request.QueryString
End Sub
...
Sub DoUpdatelog()
    ...
    Set rs = Nothing
    Set blog = Nothing
	EventLog "进行了更新日志静态页面操作，开始日志ID："&BeginID&"，结束日志ID："&EndID&"",oblog.NowUrl&"?"&Request.QueryString
End Sub
