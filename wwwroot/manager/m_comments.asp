...
<%If CheckAccess("r_user_cmt")=False Then Response.Write "无权操作":Response.End
Dim rs, sql
...
Else
    cmd = CLng(cmd)
End If
G_P_FileName = "m_comments.asp?cmd=" & cmd & "&Field=" & sField & "&keyword=" & Keyword
...
Sub Main()
    sql = "select top 500 userid,commenttopic,addtime,commentid,comment_user,addip,comment,iState,mainid,isguest From oblog_comment "
    select Case cmd
        Case 0,""
...
Sub Optcomment(sMod)
    ...
    Dim rstUser,rstCache,rstBlog,rstComment,blog,MSG
    '删除/待审/审核
    ...
    If sMod="1" Then
	    ...
	    '取单一userid
	    'Set rstUser = oblog.Execute("select b.userid,count(a.commentid) From oblog_comment a,(select userid From oblog_comment Where commentId In (" & sIds & ") Group By userid) b Where a.userid=b.userid")
	    Set rstUser = oblog.Execute("select userid From oblog_comment Where commentId In (" & sIds & ") Group By userid")
	    '取单一主logid
	    Set rstBlog = oblog.Execute("select mainid From oblog_comment Where commentid In (" & sIds & ") Group By mainid")
		PassScore sIds,sOpt
	    select Case sOpt
	        Case "1"
				WriteSysLog "进行了评论通过审核操作，目标评论ID："&sIds&"",""
	            '先给分，后处理，以获得初始状态
	            sql = "Update oblog_comment Set iState=1 Where commentId In (" & sIds & ")"
	        Case "2"
				WriteSysLog "进行了评论取消审核操作，目标评论ID："&sIds&"",""
	            sql = "Update oblog_comment Set iState=0 Where commentId In (" & sIds & ")"
	        Case "3"
				WriteSysLog "进行了评论删除操作，目标评论ID："&sIds&"",""
	...
	Else
		...
		Set rstUser = oblog.Execute("select userid From oblog_comment Where addip='" & sIP & "' Group By userid")
	...
	End If
    Dim userid,blogcomments,allComments
    '对日志评论重新进行计数
    Do While Not rstUser.EOF
		blog.userid = rstUser(0)
		blog.update_comment rstUser(0)
        '更新用户积分
        Set rstComment=oblog.Execute("select Count(commentid) From oblog_albumcomment Where isdel=0 AND istate=1 and userid=" & rstUser(0))
		allComments = rstComment(0)
        Set rstComment=oblog.Execute("select Count(commentid) From oblog_comment Where isdel=0 AND istate=1 AND userid=" & rstUser(0))
...
'        End If
    ...
    Do While Not rstBlog.Eof
    	...
    	If Not rstUser.Eof Then
	    	UserId=rstUser(0)
	    	Set rstUser=oblog.Execute("select count(commentid) From oblog_comment Where isdel=0 AND  istate=1 AND mainid=" & rstBlog(0))
	    	If rstUser.Eof Then
	    		blogcomments=0
	    	Else
...
Sub PassScore(id,iState)
	...
	End If
	If InStr(tid,",")<0 Then
		Set rs = oblog.Execute ("select userid,istate FROM oblog_comment WHERE commentid = " &tid)
		'如果是过审
		If iState=1 Then
			'只处理待审的
			If rs(1)=0 Then
				oblog.GiveScore "",sScore,rs(0)
				oblog.execute("update oblog_user set comment_count=comment_count+1 where userid="&rs(0))
		...
		Else
			...
			If rs(1)=1 Then
				oblog.GiveScore "",sScore,rs(0)
				oblog.execute("update oblog_user set comment_count=message_count-1 where userid="&rs(0))
				oblog.execute("update oblog_setup set comment_count=message_count-1")
			End if
		End If
		rs.close
	Else
		...
		For i = 0 To UBound(tid)
			Set rs = oblog.Execute ("select userid,istate FROM oblog_comment WHERE commentid = " &tid(i))
			'如果是过审
			If iState=1 Then
				'只处理待审的
				If rs(1)=0 Then
